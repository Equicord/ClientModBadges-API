...
const getAliucordBadges = async () => {
    try {
        const { data: donorData } = await axios.get("https://aliucord.com/files/badges/data.json", { headers: { "Cache-Control": "no-cache" } });
        const donors = Object.entries(donorData.users).map(([id, badges]) => {
            ...
            return { id, badges: badgeList };
        });
        donors.forEach(user => addUser(user.id, CLIENT_MODS.ALIUCORD, user.badges));
        console.log(donors);
    } catch (e) {
